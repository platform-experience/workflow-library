...
branch_checkout() {
  local branch=feature/${PREFIX}${WF}
  git fetch origin
...
make_commit() {
  ...
  cd ../../
  git add ${SRC_DIR}/${PREFIX}${WF}
  git commit -a -m "${COMMIT_MSG} ${wf_name_lower}"
...
set_name() {
  local wf_dir=()
  for i in "${inputs[@]}"; do
    wf_dir+=$(echo -${i} | tr '[:upper:]' '[:lower:]')
    wf_name_upper+=$(make_uppercase ${i})
    wf_name_lower+=$(make_lowercase ${i})
  done
  WF=$(format_data ${wf_dir[@]})
}
...
sub_base_content() {
  ...
  if [[ ${name_has_dashes} ]]; then
    ...
    replace_content "${NAME_TEMP}" "${wf_name_upper%?}" README.md
    replace_content "${NAME_TEMP}" "${wf_name_upper%?}" config.json
  fi
  replace_content "${CONTRIB_TEMP}" "$(fetch_github_user)" config.json
  replace_content "${DIR_TEMP}" "${PREFIX}${WF}" config.json
  replace_content "${DIR_TEMP}" "${PREFIX}${WF}" README.md
}
